...
export default function Header() {
  ...
  return (
    <nav className="bg-white border-b border-slate-100 sticky top-0 z-50">
      ...
      {/* MOBILE MENU */}
      {isOpen && (
        <div className="md:hidden bg-white border-b border-slate-100 flex flex-col divide-y divide-slate-50 animate-in slide-in-from-top duration-300">
          <NavLink 
            to="/" 
            onClick={() => setIsOpen(false)} 
            className="p-4 text-xs font-black uppercase tracking-widest text-slate-600 active:bg-slate-50"
          >
            Beranda
          </NavLink>
          <NavLink 
